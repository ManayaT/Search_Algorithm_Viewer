...
class Searcher:
    ...
    # <---演習部分---> Stepを書いておく
        while True:
            # Step 2
            ''' L1が空になった場合は終了となるようなコードを書く '''

            # Step 3
            # L1からノードを取り出し，L2に入れる
            node_n = '''L1から取り出す処理を記述'''
            ''' self.list_2に追加をおこなうような処理を記述 '''
            ...
            for idx in self.idx_list:
                # idx[0]とidx[1]はそれぞれy方向とx方向の移動量
                # node_nの座標にidxを加算して探索座標を計算する必要がある
                tmp_y = ''' idx[0] + ??? '''
                tmp_x = ''' idx[1] + ??? '''

                if ''' 探索座標がmaze_listの要素からはみ出していないかを判定 ''':
                    ...
                    # ノードのシンボルを取得
                    node_symbol = ''' maze_listの探索座標から,「.」や「#」などのシンボルを取得 '''
                    
                    # ノードのシンボルが "." または "*" かどうかを確認
                    if ''' self.load_symbol と self.goal_symbol を使用した条件式の記述 ''':
                        # L1とL2に重複がない場合にノードを追加
                        if ''' existing_open と existing_closed を使用した条件式の記述 ''':
